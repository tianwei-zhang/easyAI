#' Deep Learning Regression with Automated Parameter Tuning by Random Search
...
deep_lm_local=function(x,
  ...
  for(i in num_layer){
    for(j in 1:(i*iteration_per_layer)){
      ...
      cat('lr: ',lr)
      cat('\n')
      model=dl_regression_single(x,y,complexity = complexity,dropout = dropout,lr=lr,validation_split=validation_split,num_epoch = num_epoch, num_patience = num_patience)
